// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-compute-attnlike-smoke.cpp
//
// G13B smoke: backend compute-shaped callback with attention-like graph.
//
// This does not wire into full ggml_backend_i graph compute yet. Instead it
// validates the backend-owned compute callback helper over a longer graph:
//
//   ggml_cuda8_ggml_backend_dispatch_op(backend, ctx, op, src0, src1, dst)
//
...
static bool check_softmax_matrix(const std::vector<float> & got, const std::vector<float> & ref, int rows, int cols) {
    ...
    std::printf("backend compute-shaped attention probs: max_abs_err=%.9g row_sum_abs=%.9g\n", max_abs_err, max_row_sum_abs_err);
    if (max_abs_err > 3e-5 || max_row_sum_abs_err > 3e-5) {
        std::fprintf(stderr, "backend compute-shaped attention probs FAIL\n");
        return false;
    }
    std::printf("backend compute-shaped attention probs PASS\n");
    return true;
}
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-ggml-backend-compute-attnlike-smoke: starting\n");
    ...
    std::printf("backend-owned compute attention buffer size: %zu\n", buffer->size);
    ...
    if (run_backend_op(backend, ctx, GGML_CUDA8_OP_ADD_F32, &t_a_vec, &t_b_vec, &t_scores_vec, "scores = A + B") != 0 ||
        run_backend_op(backend, ctx, GGML_CUDA8_OP_MUL_SCALAR_F32, &t_scores_vec, &t_scalar, &t_scaled_vec, "scaled = scores * scale") != 0 ||
        run_backend_op(backend, ctx, GGML_CUDA8_OP_REDUCE_MAX_ROWS_F32, &t_scaled, &t_dummy, &t_row_max, "row_max = reduce_max_rows(scaled)") != 0 ||
        run_backend_op(backend, ctx, GGML_CUDA8_OP_SOFTMAX_ROWS_F32, &t_scaled, &t_dummy, &t_probs, "probs = softmax_rows(scaled)") != 0 ||
        run_backend_op(backend, ctx, GGML_CUDA8_OP_REDUCE_SUM_ROWS_F32, &t_probs, &t_dummy, &t_row_sum, "row_sum = reduce_sum_rows(probs)") != 0) {
        buffer->iface.free_buffer(buffer);
        ggml_cuda8_context_destroy(ctx);
        backend->iface.free(backend);
        return 1;
    ...
    bool ok = true;
    ok = ok && check_close_vector(row_max_out, row_max_ref, 1e-6, "backend compute attention row_max");
    ok = ok && check_softmax_matrix(probs_out, probs_ref, rows, cols);
    ok = ok && check_close_vector(row_sum_out, row_sum_ref, 3e-5, "backend compute attention row_sum(probs)");
    ...
    if (!ok) {
        std::fprintf(stderr, "ggml-cuda8-ggml-backend-compute-attnlike-smoke: FAILED\n");
        return 1;
    }

    std::printf("backend compute-shaped attention-like graph PASS\n");
    std::printf("ggml-cuda8-ggml-backend-compute-attnlike-smoke: SUCCESS\n");
    return 0;
}
