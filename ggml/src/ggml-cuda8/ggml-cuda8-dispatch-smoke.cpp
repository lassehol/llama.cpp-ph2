// ggml/src/ggml-cuda8/ggml-cuda8-dispatch-smoke.cpp
//
// G8B dispatcher smoke test.
//
// Creates:
//   - ggml_cuda8_context
//   - host packed Q8_0 matrix in ggml_cuda8_q8_0_block array
//   - host F32 vector
//   - host F32 output
// Sets up manual ggml_tensor metadata and calls dispatcher.
//
// Validates against CPU Q8_0 reference.
...
static float fp16_bits_to_fp32(uint16_t h) {
    ...
    if (h_exp == 0) {
        if (h_sig == 0) { out.u = f_sign; return out.f; }
        uint32_t sig = h_sig;
        int exp = -14;
        while ((sig & 0x0400u) == 0) { sig <<= 1; --exp; }
        sig &= 0x03FFu;
    ...
    for (int r = 0; r < rows; ++r) {
        for (int ib = 0; ib < blocks_per_row; ++ib) {
            ...
            for (int k = 0; k < GGML_CUDA8_QK8_0; ++k) {
                ...
                float v = 0.0f;
                if (col < cols) v = A[(size_t) r * cols + col];
                max_abs = std::max(max_abs, std::fabs(v));
            }

            float d = 0.0f;
            float id = 0.0f;
            if (max_abs > 0.0f) { d = max_abs / 127.0f; id = 127.0f / max_abs; }
            ...
            for (int k = 0; k < GGML_CUDA8_QK8_0; ++k) {
                ...
                int q = 0;
                if (max_abs > 0.0f) q = (int) std::floor(v * id + (v >= 0.0f ? 0.5f : -0.5f));
                if (q > 127) q = 127;
                if (q < -127) q = -127;
                block.qs[k] = (int8_t) q;
            }
...
static void cpu_q8_0_ref(
    ...
    for (int r = 0; r < rows; ++r) {
        const ggml_cuda8_q8_0_block * row_blocks = &Aq[(size_t) r * blocks_per_row];
        ...
        for (int ib = 0; ib < blocks_per_row; ++ib) {
            ...
            for (int k = 0; k < GGML_CUDA8_QK8_0; ++k) {
                ...
                if (col < cols) block_sum += ((float) block.qs[k]) * x[col];
            }
...
static void setup_tensor_q8_0_2d(
    ggml_tensor & t,
    int64_t cols,
    int64_t rows,
    void * data
    ...
    const size_t bs = sizeof(ggml_cuda8_q8_0_block);

    t.nb[0] = bs;
    t.nb[1] = (size_t) blocks_per_row * bs;
...
static void setup_tensor_f32_1d(
    ggml_tensor & t,
    int64_t n,
    void * data
) {
...
static bool run_one_case(ggml_cuda8_context * ctx, int rows, int cols) {
    std::printf("\nG8B dispatch smoke: rows=%d cols=%d\n", rows, cols);

    std::vector<float> A((size_t) rows * cols);
    std::vector<float> x((size_t) cols);
    std::vector<float> y_ref((size_t) rows, 0.0f);
    std::vector<float> y_out((size_t) rows, 0.0f);
    std::vector<ggml_cuda8_q8_0_block> Aq;
    ...
    setup_tensor_q8_0_2d(src0, cols, rows, &Aq[0]);
    setup_tensor_f32_1d(src1, cols, &x[0]);
    setup_tensor_f32_1d(dst,  rows, &y_out[0]);

    const int op = GGML_CUDA8_OP_MUL_MAT_Q8_0_F32_VEC;

    if (!ggml_cuda8_dispatch_supported(ctx, op, &src0, &src1, &dst)) {
        std::fprintf(stderr, "dispatch_supported returned false for op=%s\n", ggml_cuda8_op_name(op));
        return false;
    }

    if (ggml_cuda8_dispatch_execute(ctx, op, &src0, &src1, &dst) != 0) {
        std::fprintf(stderr, "dispatch_execute failed for op=%s\n", ggml_cuda8_op_name(op));
        return false;
    }
    ...
    compute_error(y_ref, y_out, max_abs_err, max_rel_err);
    ...
    if (max_abs_err > abs_tol && max_rel_err > rel_tol) {
        std::fprintf(stderr, "FAIL abs=%g rel=%g\n", max_abs_err, max_rel_err);
        return false;
    }
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-dispatch-smoke: starting\n");
    ...
    if (!ok) {
        std::fprintf(stderr, "ggml-cuda8-dispatch-smoke: FAILED\n");
        return 1;
    }

    std::printf("\n");
    std::printf("ggml-cuda8-dispatch-smoke: SUCCESS\n");
    return 0;
}
