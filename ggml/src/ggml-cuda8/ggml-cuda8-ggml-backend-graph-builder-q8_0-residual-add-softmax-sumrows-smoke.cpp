// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-sumrows-smoke.cpp
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-sumrows-smoke: starting\n");
    ...
    std::printf("real graph-builder packed Q8_0xF32_VEC->RESIDUAL_ADD_F32->SOFTMAX_ROWS_F32->REDUCE_SUM_ROWS_F32 max_abs_err=%.9g max_rel_err=%.9g\n",
        max_abs_err,
        max_rel_err);

    if (max_abs_err > 2e-4 && max_rel_err > 2e-4) {
        std::fprintf(stderr, "real graph-builder packed Q8_0xF32_VEC->RESIDUAL_ADD_F32->SOFTMAX_ROWS_F32->REDUCE_SUM_ROWS_F32 verification FAIL\n");
        buffer->iface.free_buffer(buffer);
        backend->iface.free(backend);
        ggml_free(gctx);
        return 1;
    ...
    std::printf("real graph-builder packed Q8_0xF32_VEC->RESIDUAL_ADD_F32->SOFTMAX_ROWS_F32->REDUCE_SUM_ROWS_F32 verification PASS\n");
    ...
    std::printf("ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-sumrows-smoke: SUCCESS\n");
    return 0;
}
