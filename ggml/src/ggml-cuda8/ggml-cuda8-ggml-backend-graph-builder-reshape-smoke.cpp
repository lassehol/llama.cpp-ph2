...
// G29A: RESHAPE/VIEW/PERMUTE/TRANSPOSE no-op support in graph_compute
// Validates that metadata-only ops are correctly skipped in the dispatch loop.
//
// Graph: x -> reshape -> permute -> add(permuted, bias) -> y
// Only ADD should be dispatched; reshape + permute are skipped.
...
int main() {
    std::printf("ggml-cuda8-ggml-backend-graph-builder-reshape-smoke: starting\n");
    ...
    // -- build graph: x -> reshape(n/2, 2) -> reshape(n) -> add(x, bias) -----
    // The reshapes are metadata-only; the final add is the only compute op.
    ...
    // reshape to 2D and back to 1D (metadata-only round-trip)
    ggml_tensor * r2d  = ggml_reshape_2d(gctx, tx, n/2, 2);
    ggml_tensor * r1d  = ggml_reshape_1d(gctx, r2d, n);
    ggml_tensor * ty   = ggml_add(gctx, r1d, bias);

    if (r2d == NULL || r1d == NULL || ty == NULL) {
    ...
    const size_t off_y    = 8192;
    const size_t total    = 12288;
    ...
    // tx and all its reshapes (r2d, r1d) share the same data pointer
    ...
    // Reshape results alias the source data
    r2d->data = base_u8 + off_x;
    r1d->data = base_u8 + off_x;

    // Register residency for real tensors
    ...
    // Reshape aliases also need residency registration
    buffer->iface.init_tensor(buffer, r2d);
    buffer->iface.init_tensor(buffer, r1d);
    ...
    for (int i = 0; i < n; ++i) {
        ...
        h_ref[i]  = h_x[i] + h_bias[i];  // reshape is identity
    }
    ...
    std::printf("real graph-builder RESHAPE->ADD max_err=%.6e\n", max_err);

    bool pass = (max_err < 1e-6f);
    std::printf("real graph-builder RESHAPE->ADD verification %s\n",
                pass ? "PASS" : "FAIL");
    ...
    std::printf("ggml-cuda8-ggml-backend-graph-builder-reshape-smoke: %s\n",
                pass ? "SUCCESS" : "FAIL");
    return pass ? 0 : 1;
}
