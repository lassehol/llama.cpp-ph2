...
static bool test_cpy(ggml_cuda8_context * ctx) {
    ...
    ggml_tensor t_src;
    ggml_tensor t_dst;
    ggml_cuda8_test_setup_f32_vector(t_src, n, &src[0]);
    ggml_cuda8_test_setup_f32_vector(t_dst, n, &dst[0]);
    ...
    if (!ggml_cuda8_test_check_exact(dst, src, "CPY_F32")) {
        return false;
    }
...
static bool test_add(ggml_cuda8_context * ctx) {
    ...
    ggml_tensor tb;
    ggml_tensor tc;
    ggml_cuda8_test_setup_f32_vector(ta, n, &a[0]);
    ggml_cuda8_test_setup_f32_vector(tb, n, &b[0]);
    ggml_cuda8_test_setup_f32_vector(tc, n, &c[0]);
    ...
    if (!ggml_cuda8_test_check_exact(c, ref, "ADD_F32")) {
        return false;
    }
...
static bool test_scalar(ggml_cuda8_context * ctx, int op, float scalar) {
    ...
    ggml_cuda8_test_setup_f32_matrix_2d(t_src, cols, rows, &src[0]);
    ggml_cuda8_test_setup_f32_scalar(t_scalar, &scalar);
    ggml_cuda8_test_setup_f32_matrix_2d(t_dst, cols, rows, &dst[0]);
    ...
    if (!ggml_cuda8_test_check_close(dst, ref, 1e-6, 1e-6, ggml_cuda8_op_name(op))) {
        return false;
    }
...
static bool test_reduce(ggml_cuda8_context * ctx, int op) {
    ...
    ggml_cuda8_test_setup_f32_matrix_2d(t_src, cols, rows, &src[0]);
    ggml_cuda8_test_setup_f32_vector(t_dst, rows, &dst[0]);
    ggml_cuda8_test_setup_f32_vector(t_dummy, 1, &dst[0]);
    ...
    if (!ggml_cuda8_test_check_close(dst, ref, 2e-4, 2e-4, ggml_cuda8_op_name(op))) {
        return false;
    }
...
static bool test_softmax(ggml_cuda8_context * ctx) {
    ...
    ggml_cuda8_test_setup_f32_matrix_2d(t_src, cols, rows, &src[0]);
    ggml_cuda8_test_setup_f32_matrix_2d(t_dst, cols, rows, &dst[0]);
    ggml_cuda8_test_setup_f32_matrix_2d(t_dummy, 1, 1, &dst[0]);
...
static bool test_q8_mul_mat(ggml_cuda8_context * ctx) {
    ...
    ggml_cuda8_test_setup_q8_0_matrix_2d(t_Aq, cols, rows, &Aq[0]);
    ggml_cuda8_test_setup_f32_vector(t_x, cols, &x[0]);
    ggml_cuda8_test_setup_f32_vector(t_y, rows, &y[0]);
    ...
    if (!ggml_cuda8_test_check_close(y, ref, 2e-4, 2e-4, "MUL_MAT_Q8_0xF32_VEC")) {
        return false;
    }
