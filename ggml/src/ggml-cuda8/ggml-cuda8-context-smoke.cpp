// ggml/src/ggml-cuda8/ggml-cuda8-context-smoke.cpp
//
// G7 CUDA8 backend context smoke.
//
// Validates:
//   - context create/destroy
//   - device info snapshot
//   - context buffer allocation/free
//   - upload/download through backend buffers
//   - Q8_0 MUL_MAT execution through context API
...
static bool run_one_case(ggml_cuda8_context * ctx, int rows, int cols) {
    std::printf("\nG7 context op smoke: rows=%d cols=%d\n", rows, cols);
    ...
    if (ggml_cuda8_context_alloc_buffer(ctx, bytes_Aq, &b_Aq) != 0) {
        ok = false;
    }

    if (ok && ggml_cuda8_context_alloc_buffer(ctx, bytes_x, &b_x) != 0) {
        ok = false;
    }

    if (ok && ggml_cuda8_context_alloc_buffer(ctx, bytes_y, &b_y) != 0) {
        ok = false;
    }

    if (ok && ggml_cuda8_backend_buffer_upload(b_Aq, 0, &Aq[0], bytes_Aq) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: Aq upload failed\n");
        ok = false;
    }

    if (ok && ggml_cuda8_backend_buffer_upload(b_x, 0, &x[0], bytes_x) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: x upload failed\n");
        ok = false;
    }

    if (ok && ggml_cuda8_backend_buffer_clear(b_y, 0) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: y clear failed\n");
        ok = false;
    }

    if (ok && ggml_cuda8_context_mul_mat_q8_0_f32(ctx, b_Aq, b_x, b_y, rows, cols) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: context mul_mat failed\n");
        ok = false;
    }

    if (ok && ggml_cuda8_backend_buffer_download(b_y, 0, &y_gpu[0], bytes_y) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: y download failed\n");
        ok = false;
    }

    ggml_cuda8_context_free_buffer(ctx, b_Aq);
    ggml_cuda8_context_free_buffer(ctx, b_x);
    ggml_cuda8_context_free_buffer(ctx, b_y);
    ...
    if (max_abs_err > abs_tol && max_rel_err > rel_tol) {
        std::fprintf(stderr,
            "ggml-cuda8-context-smoke: FAIL rows=%d cols=%d abs=%g rel=%g\n",
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-context-smoke: starting\n");

    ggml_cuda8_context * ctx = NULL;

    if (ggml_cuda8_context_create(0, &ctx) != 0) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: context create failed\n");
        return 1;
    }

    if (ctx == NULL) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: context is NULL\n");
        return 1;
    }

    if (ggml_cuda8_context_print(ctx) != 0) {
    ...
    if (!ok) {
        std::fprintf(stderr, "ggml-cuda8-context-smoke: FAILED\n");
        return 1;
    }

    std::printf("\n");
    std::printf("ggml-cuda8-context-smoke: SUCCESS\n");
    return 0;
}
