...
// G41:  SOFT_MAX_EXT (mask/scale/ALiBi) is now supported - four fixtures
//       that used to correctly document "refused, falls back to CPU" now
//       correctly document "accepted, computed on GPU". See the updated
//       comments in the FALSE-cases section below for exactly what changed
//       and why. Attention sinks and non-F32 masks remain unimplemented.
//
...
//   3. supports_op returns true for all dispatch ops + no-ops, including
//      the SOFT_MAX_EXT forms G41 now implements (mask, scale, ALiBi)
//   4. supports_op returns false for unsupported ops, including the
//      soft_max_ext forms still unimplemented (attention sinks, F16 mask)
//      that would otherwise be silently miscomputed
...
// does not apply it.
static void set_rope_params(ggml_tensor * t, int n_dims, int mode, float freq_base,
...
static ggml_tensor make_fake(ggml_type type, int64_t ne0, int64_t ne1) {
    ...
    // G41: F16 falls through to the sizeof(float) default below, which
    // understates its real element size (2 bytes, not 4). Harmless for the
    // one F16 fixture this file uses (mask_f16, added for G41) because that
    // fixture only exists to be refused by supports_op on ->type alone -
    // supports_op never dereferences ->data or relies on ->nb[] for a
    // rejected node. Flagged here rather than silently left wrong in case a
    // future F16-carrying fixture needs accurate strides.
    t.nb[0] = (type == GGML_TYPE_F32) ? sizeof(float) : (type == GGML_TYPE_I32) ? sizeof(int32_t) : sizeof(float);
    t.nb[1] = t.nb[0] * ne0;
    t.nb[2] = t.nb[1] * ne1;
    t.nb[3] = t.nb[2];
    return t;
}
static bool test_op(ggml_backend_dev_t dev, const char * label, ggml_tensor * op_t, bool expected) {
    bool result = dev->iface.supports_op(dev, op_t);
    bool pass = (result == expected);
    std::printf("  %-28s expected=%-5s got=%-5s %s\n",
                label, expected ? "true" : "false",
                result   ? "true" : "false", pass ? "PASS" : "FAIL");
    return pass;
}
int main() {
    ...
    // G41: F16 mask fixture, used only in the "F16 mask refused" rejection
    // case below. See the make_fake() comment above re: its (unused here)
    // nb[0] inaccuracy for F16.
    ggml_tensor f16_2d    = make_fake(GGML_TYPE_F16, 128, 64);
    ggml_tensor op_t;
    // NONE
    ...
    // G41: SOFT_MAX_EXT - mask only (no ALiBi). Real attention masking.
    op_t = make_fake(GGML_TYPE_F32, 64, 8);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_2d;
    op_t.src[1] = &f32_2d;  // mask
    set_soft_max_params(&op_t, 1.0f, 0.0f);
    ok &= test_op(dev, "SOFT_MAX_EXT (mask)", &op_t, true);
    // G41: SOFT_MAX_EXT - scale only, no mask. This is the actual real-
    // attention shape G39's rejection log showed dominating (soft_max_ext
    // with scale=1/sqrt(head_dim) and no mask at the un-causal-masked
    // layers, or with DIAG_MASK_INF handling causality separately).
    op_t = make_fake(GGML_TYPE_F32, 64, 1);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_64;
    set_soft_max_params(&op_t, 0.08838835f, 0.0f);
    ok &= test_op(dev, "SOFT_MAX_EXT (scale!=1, no mask)", &op_t, true);
    // G41: SOFT_MAX_EXT - mask + ALiBi max_bias together.
    op_t = make_fake(GGML_TYPE_F32, 64, 8);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_2d;
    ...
    // G37/G41: soft_max_ext forms still refused. Through G40, SOFTMAX_ROWS_F32
    // implemented only plain softmax (no mask, no sinks, scale==1,
    // max_bias==0) - every ggml_soft_max_ext() form beyond that had to be
    // refused and left to the CPU backend, since claiming them would
    // silently compute wrong attention weights (G37). G41 added
    // SOFTMAX_EXT_F32, which implements mask + scale + ALiBi max_bias - see
    // the three now-TRUE "SOFT_MAX_EXT (...)" cases above, moved out of this
    // section. Attention sinks and non-F32 masks remain unimplemented and
    // are still correctly refused here.
    // SOFT_MAX with attention sinks (src[2]) - unimplemented, refused.
    op_t = make_fake(GGML_TYPE_F32, 64, 8);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_2d;
    ...
    // G41: SOFT_MAX with a non-F32 (F16) mask - not implemented; F16 support
    // is deferred to the general F16-storage work tracked as G49. Must be
    // refused rather than silently misread as F32.
    op_t = make_fake(GGML_TYPE_F32, 64, 8);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_2d;
    op_t.src[1] = &f16_2d;  // F16 mask
    set_soft_max_params(&op_t, 1.0f, 0.0f);
    ok &= test_op(dev, "SOFT_MAX (F16 mask)", &op_t, false);
    // SOFT_MAX with zeroed op_params and no mask/sinks - a fixture that
    // forgot to stamp params reads as scale=0.0f, max_bias=0.0f. Through G40
    // this reliably fell back to CPU because is_plain() required scale==1.0f
    // exactly - the "fixture trap" this case exists to document. G41 widens
    // the accepted scale range to cover real attention's scale!=1 case (see
    // "SOFT_MAX_EXT (scale!=1, no mask)" above), and scale=0.0f with no mask
    // is not distinguishable from a deliberate (if unusual) scale=0 softmax -
    // so this specific fixture is now ALSO accepted and computed, rather
    // than refused. This is not a correctness regression (the kernel
    // computes the literal scale=0 case correctly: a uniform distribution
    // over the row), but this fixture no longer catches an unstamped-params
    // mistake the way it used to - any future SOFT_MAX fixture must still
    // stamp params deliberately if it means to test a *specific*
    // scale/max_bias value rather than relying on this case to catch the
    // omission.
    op_t = make_fake(GGML_TYPE_F32, 64, 1);
    op_t.op = GGML_OP_SOFT_MAX;
    op_t.src[0] = &f32_64;
    ok &= test_op(dev, "SOFT_MAX (zero params, now valid scale=0)", &op_t, true);
