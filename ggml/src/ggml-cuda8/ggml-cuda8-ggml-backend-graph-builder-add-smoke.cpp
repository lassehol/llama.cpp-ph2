// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-graph-builder-add-smoke.cpp
// G16A smoke: real GGML graph-builder ADD graph -> CUDA8 backend graph_compute.
...
int main(int argc, char ** argv) {
    (void) argc;
    (void) argv;
    std::printf("ggml-cuda8-ggml-backend-graph-builder-add-smoke: starting\n");
    ...
    if (graph->n_nodes < 1) {
        std::fprintf(stderr, "real ggml graph unexpectedly has no nodes\n");
        ggml_free(gctx);
        return 1;
    }
    ...
    std::vector<float> a_host(n), b_host(n), c_ref(n), c_out(n, 0.0f);
    for (int i = 0; i < n; ++i) {
    ...
    std::printf("real graph-builder ADD_F32 max_abs_err=%.9g\n", max_abs_err);
    if (max_abs_err > 1e-6) {
        std::fprintf(stderr, "real graph-builder ADD_F32 verification FAIL\n");
        buffer->iface.free_buffer(buffer);
        backend->iface.free(backend);
        ggml_free(gctx);
        return 1;
    }
    std::printf("real graph-builder ADD_F32 verification PASS\n");

    buffer->iface.free_buffer(buffer);
    backend->iface.free(backend);
    ggml_free(gctx);
    std::printf("ggml-cuda8-ggml-backend-graph-builder-add-smoke: SUCCESS\n");
    return 0;
}
