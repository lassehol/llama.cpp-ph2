// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-buffer-graph-smoke.cpp
//
// G12B smoke: backend-owned buffer graph smoke.
//
// Validates:
//   - minimal CUDA8 ggml_backend_t can be created
//   - backend default buffer type helper returns CUDA8 buffer type
//   - graph buffer is allocated through backend default buffer type
//   - tensors in that backend-owned buffer are CUDA8-resident
//   - existing residency-aware dispatcher can run a device-resident graph
//
// Graph:
//   C = A + B
//   D = C * scalar
...
static bool check_softmax_matrix(const std::vector<float> & got, const std::vector<float> & ref, int rows, int cols) {
    ...
    std::printf("backend-owned softmax graph: max_abs_err=%.9g row_sum_abs=%.9g\n", max_abs_err, max_row_sum_abs_err);
    if (max_abs_err > 3e-5 || max_row_sum_abs_err > 3e-5) {
        std::fprintf(stderr, "backend-owned softmax graph FAIL\n");
        return false;
    }
    std::printf("backend-owned softmax graph PASS\n");
    return true;
}
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-ggml-backend-buffer-graph-smoke: starting\n");
    ...
    if (off_a + matrix_bytes > total_size ||
        ...
        off_c + matrix_bytes > total_size ||
        off_d + matrix_bytes > total_size ||
        off_s + matrix_bytes > total_size ||
    ...
    std::printf("backend graph op 1: C = A + B PASS\n");
    ...
    std::printf("backend graph op 2: D = C * scalar PASS\n");
    ...
    std::printf("backend graph op 3: S = softmax_rows(D) PASS\n");
    ...
    std::printf("backend-owned device graph PASS\n");

    buffer->iface.free_buffer(buffer);
    ggml_cuda8_context_destroy(ctx);
    backend->iface.free(backend);

    std::printf("ggml-cuda8-ggml-backend-buffer-graph-smoke: SUCCESS\n");
    return 0;
}
