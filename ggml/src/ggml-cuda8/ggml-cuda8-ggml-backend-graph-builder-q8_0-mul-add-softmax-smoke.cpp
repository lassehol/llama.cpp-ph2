// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-smoke.cpp
...
int main(int argc, char ** argv) {
    ...
    std::printf("ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-smoke: starting\n");
    ...
    if (graph->n_nodes != 4) {
        std::fprintf(stderr, "expected 4 graph nodes, got %d\n", graph->n_nodes);
        ggml_free(gctx);
        return 1;
    }
    ...
    std::printf("real graph-builder packed Q8_0xF32_VEC->MUL_SCALAR_F32->ADD_F32->SOFTMAX_ROWS_F32 max_abs_err=%.9g max_rel_err=%.9g\n",
        max_abs_err,
        max_rel_err);

    if (max_abs_err > 2e-4 && max_rel_err > 2e-4) {
        std::fprintf(stderr, "real graph-builder packed Q8_0xF32_VEC->MUL_SCALAR_F32->ADD_F32->SOFTMAX_ROWS_F32 verification FAIL\n");
        buffer->iface.free_buffer(buffer);
        backend->iface.free(backend);
        ggml_free(gctx);
        return 1;
    ...
    std::printf("real graph-builder packed Q8_0xF32_VEC->MUL_SCALAR_F32->ADD_F32->SOFTMAX_ROWS_F32 verification PASS\n");

    buffer->iface.free_buffer(buffer);
    backend->iface.free(backend);
    ggml_free(gctx);

    std::printf("ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-smoke: SUCCESS\n");
    return 0;
}
