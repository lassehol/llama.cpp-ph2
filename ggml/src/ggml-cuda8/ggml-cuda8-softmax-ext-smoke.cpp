...
//   dst[c] = softmax(v)[c]
// with the standard ALiBi slope construction (see ggml-cuda8-softmax-ext.cu's
// header comment for the exact formula).
//
// Compiled as plain C++ (.cpp), NOT .cu: this file contains no device code
// (no __global__, no <<<>>> launches) - only host-side CUDA runtime API
// calls and a call into the extern "C" launcher already compiled into
// ggml-cuda8-kernels by nvcc. It needs the full ggml_tensor/ggml_cuda8_context
// struct definitions from ggml.h / ggml-cuda8-context.h to build the
// supported()-rejection test cases, and those headers do not parse cleanly
// under nvcc's front end - the same reason ggml-cuda8-add-smoke,
// ggml-cuda8-softmax-smoke, and the other struct-consuming smokes in this
// directory are also .cpp rather than .cu. Standalone kernel-only smokes
// that never touch these headers (ggml-cuda8-oversized-smoke.cu,
// ggml-cuda8-rope-smoke.cu, ...) stay .cu and declare their launchers via a
// bare extern "C" prototype instead.
