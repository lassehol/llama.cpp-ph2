// ggml/src/ggml-cuda8/ggml-cuda8-ggml-buffer-device-attnlike-smoke.cpp
//
// G11E smoke: attention-like device-resident micrograph.
//
// Graph, all tensors inside one CUDA8 ggml_backend_buffer_t:
//   scores = A + B
//   scaled = scores * scale
...
//
// Only final verification readbacks are copied to host.
...
static bool check_softmax_matrix(const std::vector<float> & got, const std::vector<float> & ref, int rows, int cols) {
    ...
    std::printf("attention-like probs: max_abs_err=%.9g row_sum_abs=%.9g\n", max_abs_err, max_row_sum_abs_err);
    if (max_abs_err > 3e-5 || max_row_sum_abs_err > 3e-5) {
        std::fprintf(stderr, "attention-like probs FAIL\n");
        return false;
    }
    std::printf("attention-like probs PASS\n");
    return true;
}
...
int main(int argc, char ** argv) {
    (void) argc;
    (void) argv;
    std::printf("ggml-cuda8-ggml-buffer-device-attnlike-smoke: starting\n");
    ...
    ggml_tensor t_a, t_b, t_scores, t_scaled, t_probs;
    ggml_tensor t_row_max, t_row_sum, t_scalar, t_dummy;
    ggml_tensor t_a_vec, t_b_vec, t_scores_vec, t_scaled_vec;
    ...
    std::vector<float> a_host(n), b_host(n), scores_ref(n), scaled_ref(n), probs_ref(n), probs_out(n, 0.0f);
    std::vector<float> row_max_ref(rows, -FLT_MAX), row_sum_ref(rows, 1.0f), row_max_out(rows, 0.0f), row_sum_out(rows, 0.0f);
    ...
    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_ADD_F32, &t_a_vec, &t_b_vec, &t_scores_vec)) return 1;
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_ADD_F32, &t_a_vec, &t_b_vec, &t_scores_vec) != 0) return 1;
    std::printf("attention op 1: scores = A + B PASS\n");

    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_MUL_SCALAR_F32, &t_scores_vec, &t_scalar, &t_scaled_vec)) return 1;
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_MUL_SCALAR_F32, &t_scores_vec, &t_scalar, &t_scaled_vec) != 0) return 1;
    std::printf("attention op 2: scaled = scores * scale PASS\n");

    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_REDUCE_MAX_ROWS_F32, &t_scaled, &t_dummy, &t_row_max)) return 1;
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_REDUCE_MAX_ROWS_F32, &t_scaled, &t_dummy, &t_row_max) != 0) return 1;
    std::printf("attention op 3: row_max = reduce_max_rows(scaled) PASS\n");

    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_SOFTMAX_ROWS_F32, &t_scaled, &t_dummy, &t_probs)) return 1;
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_SOFTMAX_ROWS_F32, &t_scaled, &t_dummy, &t_probs) != 0) return 1;
    std::printf("attention op 4: probs = softmax_rows(scaled) PASS\n");

    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_REDUCE_SUM_ROWS_F32, &t_probs, &t_dummy, &t_row_sum)) return 1;
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_REDUCE_SUM_ROWS_F32, &t_probs, &t_dummy, &t_row_sum) != 0) return 1;
    std::printf("attention op 5: row_sum = reduce_sum_rows(probs) PASS\n");
    ...
    bool ok = true;
    ok = ok && check_close_vector(row_max_out, row_max_ref, 1e-6, "attention row_max");
    ok = ok && check_softmax_matrix(probs_out, probs_ref, rows, cols);
    ok = ok && check_close_vector(row_sum_out, row_sum_ref, 3e-5, "attention row_sum(probs)");

    buffer->iface.free_buffer(buffer);
    ggml_cuda8_context_destroy(ctx);
    if (!ok) {
        std::fprintf(stderr, "ggml-cuda8-ggml-buffer-device-attnlike-smoke: FAILED\n");
        return 1;
    }
    std::printf("ggml-cuda8-ggml-buffer-device-attnlike-smoke: SUCCESS\n");
    return 0;
}
