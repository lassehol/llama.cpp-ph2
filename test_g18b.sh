...
grep -q "CUDA8 G18B regression SUCCESS" ./run_g11_regression.sh

grep -q "G18 status: real GGML graph-builder quantized pipeline coverage" ggml/src/ggml-cuda8/README.md
grep -q "G18A" ggml/src/ggml-cuda8/README.md
...
echo "G18B metadata checks PASS"

if [ -x ./test_g18a.sh ]; then
    ./test_g18a.sh
else
    echo "test_g18a.sh not found; running main regression instead"
    chmod +x ./run_g11_regression.sh
    ./run_g11_regression.sh
fi

echo
echo "G18B regression + README status SUCCESS"
