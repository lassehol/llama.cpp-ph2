...
grep -q "CUDA8 G22B regression SUCCESS" ./run_g11_regression.sh

grep -q "G22 status: real GGML graph-builder quantized residual softmax coverage" ggml/src/ggml-cuda8/README.md
grep -q "G22A" ggml/src/ggml-cuda8/README.md
grep -q "G22B" ggml/src/ggml-cuda8/README.md
...
echo "G22B metadata checks PASS"

if [ -x ./test_g22a.sh ]; then
    ./test_g22a.sh
else
    echo "test_g22a.sh not found; running main regression instead"
    chmod +x ./run_g11_regression.sh
    ./run_g11_regression.sh
fi

echo
echo "G22B regression + README status SUCCESS"
