...
# G23A_writer.py
#
# Create G23A real graph-builder residual-softmax-sumrows quantized pipeline smoke:
#
#   h       = mul_mat(Q8_0, x)
#   biased  = h + residual
#   prob    = softmax(biased)
...
#
# This derives from the validated G22A Q8_0 -> residual ADD -> SOFTMAX smoke and
# preserves the residual input isolation check after graph_compute.
#
...
SRC_IN = os.path.join(BASE, "ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-smoke.cpp")
SRC_OUT = os.path.join(BASE, "ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-sumrows-smoke.cpp")
...
OLD_NAME = "ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-smoke"
NEW_NAME = "ggml-cuda8-ggml-backend-graph-builder-q8_0-residual-add-softmax-sumrows-smoke"
...
if "static void sum_ref(" not in s:
...
# Tensor creation: add prob intermediate and make y=sum_rows(prob).
s = sub_once(
    s,
    ...
    r'    ggml_tensor \* y\s*=\s*ggml_soft_max\(gctx, biased\);\n\n'
    r'    if \(Aq == NULL \|\| x == NULL \|\| h == NULL \|\| residual == NULL \|\| biased == NULL \|\| y == NULL\) \{',
    r'\1'
    '    ggml_tensor * prob     = ggml_soft_max(gctx, biased);\n'
    '    ggml_tensor * y        = ggml_sum_rows(gctx, prob);\n\n'
    '    if (Aq == NULL || x == NULL || h == NULL || residual == NULL || biased == NULL || prob == NULL || y == NULL) {',
    "tensor creation"
)

s = s.replace("graph->n_nodes != 3", "graph->n_nodes != 4", 1)
s = s.replace("expected 3 graph nodes", "expected 4 graph nodes", 1)

# Offsets: insert prob vector and compact sum_rows scalar output.
s = sub_once(
    s,
    ...
    r'    const size_t off_y\s*=\s*24576;\n'
    r'    const size_t total_size\s*=\s*28672;\n',
    ...
    '    const size_t off_prob     = 24576;\n'
    '    const size_t off_y        = 28672;\n'
    '    const size_t total_size = 28928;\n',
    "offset block"
)
...
# Layout: y is compact one-F32 output now.
s = sub_once(
    s,
...
# Residency: prob rows, y one float.
s = sub_once(
    s,
...
# CPU reference: y_ref/y_out are scalar now, prob_ref is rows.
s = sub_once(
    s,
    ...
    r'    softmax_ref\(biased_ref, y_ref\);\n',
...
s = sub_once(
    s,
    r'    buffer->iface.get_tensor\(buffer, y, &y_out\[0\], 0, bytes_y\);\n',
    '    buffer->iface.get_tensor(buffer, y, &y_out[0], 0, sizeof(float));\n',
    "get y"
)

# Verification loop should iterate y_ref size, not rows.
s = sub_once(
    s,
    r'    for \(int i = 0; i < rows; \+\+i\) \{\n'
    r'        const double r = \(double\) y_ref\[i\];\n'
    r'        const double g = \(double\) y_out\[i\];\n\n'
    r'        const double abs_err = std::fabs\(g - r\);\n'
    r'        const double rel_err = abs_err / std::max\(1e-9, std::fabs\(r\)\);\n\n'
    r'        if \(abs_err > max_abs_err\) max_abs_err = abs_err;\n'
    r'        if \(rel_err > max_rel_err\) max_rel_err = rel_err;\n'
    r'    \}\n',
