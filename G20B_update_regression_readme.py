...
#
# G20B: refresh main CUDA8 regression script + README status for G20A.
#
# Updates:
...
def insert_quant_pipeline_targets(data):
    ...
    raise RuntimeError("could not find safe insertion anchor for G17/G18/G19/G20 targets")
...
def update_success_label(data):
    ...
    for label in labels:
        if label in data:
            data = data.replace(label, 'echo "CUDA8 G20B regression SUCCESS"')
            found = True
...
def refresh_regression():
    ...
    if data != orig:
        ...
        print("main regression already current for G20B")
...
def refresh_readme():
    ...
    if data != orig:
        ...
        print("README G20 status already current")


def main():
    refresh_regression()
    refresh_readme()
    print("G20B regression + README refresh complete.")
