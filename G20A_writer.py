...
# G20A_writer.py
#
# Create G20A real graph-builder five-op quantized pipeline smoke:
#
#   h       = mul_mat(Q8_0, x)
#   scaled  = h * scale
#   biased  = scaled + bias
...
#
# The writer derives from the validated G19A Q8_0 -> MUL_SCALAR -> ADD -> SOFTMAX smoke.
...
SRC_IN = os.path.join(BASE, "ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-smoke.cpp")
SRC_OUT = os.path.join(BASE, "ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-sumrows-smoke.cpp")
CMAKE = os.path.join(BASE, "CMakeLists.txt")

TARGET = "ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-sumrows-smoke"

OLD_NAME = "ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-smoke"
NEW_NAME = "ggml-cuda8-ggml-backend-graph-builder-q8_0-mul-add-softmax-sumrows-smoke"
...
s = s.replace(
    "G19A: real GGML graph-builder Q8_0 MUL_MAT -> MUL_SCALAR -> ADD -> SOFTMAX pipeline through CUDA8 graph_compute.",
    "G20A: real GGML graph-builder Q8_0 MUL_MAT -> MUL_SCALAR -> ADD -> SOFTMAX -> SUM_ROWS pipeline through CUDA8 graph_compute."
)
...
old = '''    ggml_tensor * Aq     = ggml_new_tensor_2d(gctx, GGML_TYPE_Q8_0, cols, rows);
    ...
    ggml_tensor * biased = ggml_add(gctx, scaled, bias);
    ggml_tensor * y      = ggml_soft_max(gctx, biased);

    if (Aq == NULL || x == NULL || h == NULL || scale == NULL || scaled == NULL || bias == NULL || biased == NULL || y == NULL) {
'''
new = '''    ggml_tensor * Aq      = ggml_new_tensor_2d(gctx, GGML_TYPE_Q8_0, cols, rows);
    ...
    if (Aq == NULL || x == NULL || h == NULL || scale == NULL || scaled == NULL || bias == NULL || biased == NULL || prob == NULL || y == NULL) {
'''
...
old = '''    const size_t off_Aq     = 0;
    ...
    const size_t off_bias   = 20480;
    const size_t off_biased = 24576;
    const size_t off_y      = 28672;
    const size_t total_size = 32768;
'''
...
s = s.replace(
    'std::printf("backend-owned q8_0 mul-add-softmax pipeline buffer size: %zu\\n", buffer->size);',
    'std::printf("backend-owned q8_0 mul-add-softmax-sumrows pipeline buffer size: %zu\\n", buffer->size);'
)
...
old = '''    std::vector<float> A_f32;
    ...
    add_ref(scaled_ref, bias_host, biased_ref);
...
'''
new = '''    buffer->iface.get_tensor(buffer, y, &y_out[0], 0, sizeof(float));
'''
s = replace_once(s, old, new, "get tensor")

old = '''    for (int i = 0; i < rows; ++i) {
...
'''
new = '''    for (size_t i = 0; i < y_ref.size(); ++i) {
        const double r = (double) y_ref[i];
        const double g = (double) y_out[i];
...
'''
s = replace_once(s, old, new, "verification loop")

s = s.replace(
    "real graph-builder packed Q8_0xF32_VEC->MUL_SCALAR_F32->ADD_F32->SOFTMAX_ROWS_F32",
    "real graph-builder packed Q8_0xF32_VEC->MUL_SCALAR_F32->ADD_F32->SOFTMAX_ROWS_F32->REDUCE_SUM_ROWS_F32"
)
