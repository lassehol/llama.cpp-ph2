...
#
# G18D: refresh main CUDA8 regression script + README status for G18C.
#
# Updates:
...
def update_success_label(data):
    ...
    for label in labels:
        if label in data:
            data = data.replace(label, 'echo "CUDA8 G18D regression SUCCESS"')
            found = True
...
def refresh_regression():
    ...
    if data != orig:
        ...
        print("main regression already current for G18D")
...
def main():
    refresh_regression()
    refresh_readme()
    print("G18D regression + README refresh complete.")
