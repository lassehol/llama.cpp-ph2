...
#
# Patches:
#   ggml/src/ggml-cuda8/CMakeLists.txt
#
...
SRC = os.path.join(BASE, "ggml-cuda8-ggml-backend-graph-builder-q8_0-mmv-smoke.cpp")
CMAKE = os.path.join(BASE, "CMakeLists.txt")
TARGET = "ggml-cuda8-ggml-backend-graph-builder-q8_0-mmv-smoke"

CPP = r'''// ggml/src/ggml-cuda8/ggml-cuda8-ggml-backend-graph-builder-q8_0-mmv-smoke.cpp
// G17A: real GGML graph-builder Q8_0 x F32 vector smoke through CUDA8 graph_compute.
//
    ...
    for (int r = 0; r < rows; ++r) {
        for (int ib = 0; ib < blocks_per_row; ++ib) {
            ...
            for (int k = 0; k < GGML_CUDA8_QK8_0; ++k) {
                ...
                int q = 0;
                if (c < cols) {
                    // Small deterministic signed values.
                    q = ((r * 17 + c * 31 + 7) % 17) - 8;
                }
...
static void cpu_ref_q8_mmv(
    ...
    for (int r = 0; r < rows; ++r) {
        ...
        for (int ib = 0; ib < blocks_per_row; ++ib) {
            ...
            // block.d is 1.0f by construction.
            const float d = 1.0f;
...
int main(int argc, char ** argv) {
    ...
    std::printf("real graph-builder Q8_0xF32_VEC max_abs_err=%.9g max_rel_err=%.9g\n",
        max_abs_err,
        max_rel_err);

    if (max_abs_err > 2e-4 && max_rel_err > 2e-4) {
        std::fprintf(stderr, "real graph-builder Q8_0xF32_VEC verification FAIL\n");
        buffer->iface.free_buffer(buffer);
        backend->iface.free(backend);
        ggml_free(gctx);
        return 1;
    ...
    std::printf("real graph-builder Q8_0xF32_VEC verification PASS\n");
