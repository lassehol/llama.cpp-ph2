...
grep -q "CUDA8 G21B regression SUCCESS" ./run_g11_regression.sh

grep -q "G21 status: real GGML graph-builder quantized residual branch coverage" ggml/src/ggml-cuda8/README.md
grep -q "G21A" ggml/src/ggml-cuda8/README.md
grep -q "G21B" ggml/src/ggml-cuda8/README.md
...
echo "G21B metadata checks PASS"

if [ -x ./test_g21a.sh ]; then
    ./test_g21a.sh
else
    echo "test_g21a.sh not found; running main regression instead"
    chmod +x ./run_g11_regression.sh
    ./run_g11_regression.sh
fi

echo
echo "G21B regression + README status SUCCESS"
