...
#
# G21B: refresh main CUDA8 regression script + README status for G21A.
#
# Updates:
...
def insert_quant_pipeline_targets(data):
    ...
    raise RuntimeError("could not find safe insertion anchor for G17/G18/G19/G20/G21 targets")
...
def update_success_label(data):
    ...
    for label in labels:
        if label in data:
            data = data.replace(label, 'echo "CUDA8 G21B regression SUCCESS"')
            found = True
...
def refresh_regression():
    ...
    if data != orig:
        ...
        print("main regression already current for G21B")
...
def refresh_readme():
    ...
    if data != orig:
        ...
        print("README G21 status already current")


def main():
    refresh_regression()
    refresh_readme()
    print("G21B regression + README refresh complete.")
