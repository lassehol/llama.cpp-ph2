...
python3 ./G19B_update_regression_readme.py
...
grep -q "CUDA8 G19B regression SUCCESS" ./run_g11_regression.sh

grep -q "G19 status: real GGML graph-builder quantized softmax pipeline coverage" ggml/src/ggml-cuda8/README.md
grep -q "G19A" ggml/src/ggml-cuda8/README.md
grep -q "G19B" ggml/src/ggml-cuda8/README.md
...
echo "G19B metadata checks PASS"

if [ -x ./test_g19a.sh ]; then
    ./test_g19a.sh
else
    echo "test_g19a.sh not found; running main regression instead"
    chmod +x ./run_g11_regression.sh
    ./run_g11_regression.sh
fi

echo
echo "G19B regression + README status SUCCESS"
