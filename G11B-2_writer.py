...
BASE = "/workspace/notebooks/llamaset(&t, 0, sizeof(t));BASE = "/workspace/notebooks/llama.cpp-ph2/ggml/src/ggml-cuda8"
...
int main(int argc, char ** argv) {
    ...
    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_ADD_F32, &t_a, &t_b, &t_c)) {
    ...
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_ADD_F32, &t_a, &t_b, &t_c) != 0) {
    ...
    if (!ggml_cuda8_dispatch_supported(ctx, GGML_CUDA8_OP_MUL_SCALAR_F32, &t_c, &t_scalar, &t_d)) {
    ...
    if (ggml_cuda8_dispatch_execute(ctx, GGML_CUDA8_OP_MUL_SCALAR_F32, &t_c, &t_scalar, &t_d) != 0) {
...
if "ggml-cuda8-ggml-buffer-device-softmax-graph-smoke" not in cm:
    if not cm.endswith("\n"):
        cm += "\n"
    cm += target
...
print("")
print("G11B-2 writer complete.")

SMOKE = os.path.join(BASE, "ggml-cuda8-ggml-buffer-device-softmax-graph-smoke.cpp")
CMAKE = os.path.join(BASE, "CMakeLists.txt")


def backup(path):
    if os.path.exists(path):
        b = path + ".g11b2-backup-" + str(int(time.time()))
...
write(SMOKE, r'''// ggml/src/ggml-cuda8/ggml-cuda8-ggml-buffer-device-softmax-graph-smoke.cpp
//
